...
## v2 — Plugin como unidade completa
Fatiado (cada fatia = um ciclo `/feature`):

- **v2a — Subagents** *(em andamento)*: CRUD + geração por IA; espelho em
  `~/.claude/agents/<name>.md`; export no bundle (`agents/`).
  Caso de uso: `qa`/`code-review` como subagent em Haiku read-only (corte de custo).
- **v2b — MCP-usage**: vincular uma skill a um MCP ("como usar bem o MCP") +
  `.mcp.json` no bundle. Barato e alto valor.
- **v2c — Hooks**: CRUD (evento + matcher + comando) → `hooks/hooks.json` no
  bundle. ⚠️ espelhar em `settings.json` é o único ponto destrutivo (merge +
  backup + idempotência) — tratar como ação crítica, opt-in.
- **v2d — Commands + polimento**: slash commands no bundle; UI de plugin com
  abas por construto; versionamento/changelog do plugin.
