...
import { useMcpImport, useMcpStoreSearch } from "@/hooks/useMcpStore"
...
export function McpStore() {
  ...
  const { data: servers, isFetching, isError } = useMcpStoreSearch(ativo, true)
  ...
  return (
    <Modal onClose={onClose} title={server.title || server.suggested_name} size="lg">
      <div className="stack" style={{ gap: 12 }}>
        ...
        {server.description && <p className="sub" style={{ margin: 0 }}>{server.description}</p>}
        ...
        <div className="row wrap" style={{ gap: 6 }}>
          ...
          {server.source_url && (
            <a href={server.source_url} target="_blank" rel="noreferrer" className="tag" style={{ textDecoration: "none" }}>
              <Icon name="external" size={12} /> origem
            </a>
          )}
        </div>
